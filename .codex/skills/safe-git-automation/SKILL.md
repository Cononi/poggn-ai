...
사용자의 변경을 잃거나 섞지 않고 Git 작업을 재현 가능하게 수행한다.
...
Single Agent와 Multi Agent 모두 동일 규칙을 적용하고, merge/push/release 판단은 deterministic script 결과를 우선해 LLM은 결과 요약 역할로 제한한다.
...
`git commit`, `git push`, `git merge`, `git all` auto/once 옵션은 commit/push/merge 자동화만 포함하며 release를 포함하지 않는다.
release는 auto 대상이 아니며, 현재 사용자 요청에 `릴리즈 처리`, `release 생성`, `배포/릴리즈까지` 같은 명시가 있을 때만 실행한다.
...
release note 초안/검증은 `.codex/script/pogo_release.py status/notes/merge-notes` 출력 1회와 `pogo-state/subagent-reports/**` 근거를 기준으로 subagent가 요약 검토한다.
메인 오케스트레이터는 release 명시 요청 여부, 보호 규칙 위반 여부, subagent 요약의 차단 사유만 검수한다.
...
### worktree 없이 진행해도 되는 경우

- 설명, 질문 답변, read-only 확인, 단순 상태 조회처럼 파일 변경이나 branch 전환이 필요 없는 작업.
- 현재 workspace가 clean이고 단일 파일의 작은 문구 수정처럼 충돌 위험이 낮은 작업.
- 현재 작업공간에 이미 사용자가 만든 대상 미커밋 변경이 있고, 그 변경을 그대로 commit/release해야 하는 작업.

### worktree가 필요한 경우

- 여러 파일 또는 여러 project path를 수정해 변경 범위가 섞일 수 있는 작업.
- release 준비, main 반영, 장시간 검증처럼 최종 전달 단계까지 이어질 가능성이 높은 작업.
...
- fast-forward가 가능한 기본 경로를 먼저 사용한다.
- 보존 규칙이 merge record를 요구하면 `title/body/footer` merge 템플릿을 사용해 merge commit을 남기고, 해당 merge commit hash와 검증 상태를 기록한다.
- merge record 사용은 필요 최소 범위로 제한하고, `--admin`/`--force`는 명시 승인 없으면 금지한다.
...
main merge 후에도 release는 현재 사용자 요청에 명시적 release 지시가 있을 때만 수행한다.
`merge`, `push`, `git all`, auto/once 옵션은 release 승인을 대신하지 않는다.
release는 GitHub Releases 페이지에 추적 가능한 기록으로 남긴다.

release 자동 생성은 기본적으로 수행하지 않는다. 먼저 `릴리즈 필요성 판단`으로 권고만 제시하고, 사용자가 명시적으로 `릴리즈 처리 해주세요`라고 승인한 경우에만 실행한다.
worktree 생성, 중간 수정, 중간 commit, branch push는 release 조건이 아니다. 같은 주제의 수정이 반복될 가능성이 있으면 release하지 않고 최종안 확정까지 worktree 또는 통합 branch에서 유지한다. release는 검증된 최종안이 `main`에 반영된 뒤 project/version/tag 기준으로 한 번만 생성한다.
...
1. worktree는 작업 시작과 반복 수정 공간으로만 사용하고, 생성 시점에는 release하지 않는다.
2. 로컬 검증은 한 번만 실행하고 결과를 commit footer와 release note에 재사용한다.
3. 작업 branch push 후 동일 SHA의 required check를 한 번만 확인한다.
4. 최종안 확정 전에는 같은 주제의 중간 commit이나 branch push마다 release하지 않는다.
5. `main`은 `--ff-only`로만 반영하고, main push 후 같은 SHA의 required check를 한 번만 확인한다.
6. release 대상 project, version source, 최신 tag는 release 생성 직전에 한 번만 확인한다.
7. release note 생성에는 `--state-report`로 `pogo-state/subagent-reports/**` 근거를 최소 1개 이상 연결한다.
8. GitHub Release 제목은 버전/tag로 작성하고, 핵심 변경 내용은 본문 요약에 작성한다.
9. release 생성 후 URL과 remote tag SHA만 확인하고, 같은 조회를 반복하지 않는다.

금지되는 지연 패턴:

- 같은 SHA에 대해 `gh run list/view`, `git rev-parse`, `git status`를 이유 없이 반복 실행.
- `status`, `impacted`, `notes`, `merge-notes`를 각각 따로 반복 호출해 동일한 diff 범위를 재계산.
- worktree 생성, 중간 수정, 중간 commit, branch push마다 release 생성.
- `pogo-state` 리포트를 읽지 않고 commit subject와 파일명만으로 release note 작성.
- 동일 범위의 `git diff`, `git log`, `gh run`, `pogo-state` 대용량 출력, raw release body를 목적 변경 없이 반복 출력.
- release body, full diff, raw log를 2회 이상 재열람해야 할 때 원본 전체 대신 명령명, run id, commit/tag, report path만 재인용한다.

권장 순서:

1. 검증된 branch commit이 main에 fast-forward로 반영되었는지 remote에서 확인한다.
...
5. `.codex/script/pogo_release.py status --project <project> --path <path>`와 `notes`, `merge-notes`로 project 기준 release note 초안을 확인한다.
...
Release note는 다음 항목을 포함한다.
각 항목은 유지보수자와 인수인계자가 과거 맥락 없이도 흐름을 따라갈 수 있을 정도로 구체적으로 작성한다.
특히 버전업은 단순 숫자 변경이 아니라 호환성, 배포, 롤백 기준을 바꾸는 감사 대상이므로 이전/현재 버전과 버전업 이유를 반드시 기록한다.
