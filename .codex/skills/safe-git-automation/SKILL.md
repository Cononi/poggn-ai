...
1. 로컬 검증은 한 번만 실행하고 결과를 commit footer와 release note에 재사용한다.
2. 작업 branch push 후 동일 SHA의 required check를 한 번만 확인한다.
3. `main`은 `--ff-only`로만 반영하고, main push 후 같은 SHA의 required check를 한 번만 확인한다.
4. release 대상 project, version source, 최신 tag는 release 생성 직전에 한 번만 확인한다.
5. release note 생성에는 `--state-report`로 `pogo-state/subagent-reports/**` 근거를 최소 1개 이상 연결한다.
6. GitHub Release 제목은 버전/tag로 작성하고, 핵심 변경 내용은 본문 요약에 작성한다.
7. release 생성 후 URL과 remote tag SHA만 확인하고, 같은 조회를 반복하지 않는다.
