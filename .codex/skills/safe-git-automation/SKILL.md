...
description: Git 변경을 안전하게 분기·검증·커밋·푸시·PR로 연결한다. 병렬 작업과 worktree 판단이 필요할 때 사용한다.
metadata:
  version: "1.0"
---
...
- push 또는 PR을 만들 때: `references/pr-template.md`
...
- 성공하지 않은 push, PR, merge를 완료라고 보고하지 않는다.
...
commit body, PR 본문, release note는 `lang=ko`일 때 한국어로 작성할 수 있다.
...
5. 검증 실패를 숨긴 채 commit, push, PR을 완료 처리하지 않는다.
...
PR 본문에는 Issue, Spec, 실제 변경, 검증 결과, 위험과 롤백을 연결한다.

상세 형식은 `references/pr-template.md`를 따른다.
...
1. PR merge 또는 main 반영을 원격에서 확인한다.
...
PR 생성 직후 worktree를 자동 삭제하지 않는다. 리뷰 수정에 다시 사용할 수 있게 유지한다.
...
- commit: 실제 변경 단위 기록
- PR: Issue와 `docs/specs/.../spec.md` 링크
...
- `PUBLISH`: remote 반영과 PR URL이 확인됨
- `DELIVER`: merge 또는 release 결과가 원격에서 확인됨
