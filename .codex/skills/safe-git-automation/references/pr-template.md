...
worktree 생성, 중간 수정, 중간 commit, branch push는 release 조건이 아니다. 같은 주제의 수정이 반복되면 최종안이 `main`에 반영될 때까지 release하지 않는다.
속도 때문에 같은 확인을 반복하지 않는다. 로컬 검증 1회, branch required check 1회, main required check 1회, release 생성 후 URL/tag 확인 1회가 표준이다.
release title은 버전/tag로 두고, 핵심 변경 내용과 `pogo-state/subagent-reports` 근거는 본문에 포함한다.
...
- release note에 `pogo-state/subagent-reports` 근거가 없음
- worktree 생성, 중간 수정, 중간 commit, branch push 같은 최종안 이전 상태를 release하려고 함
