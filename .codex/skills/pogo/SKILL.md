...
2. `작업 진행 예정 보고서`를 낸 뒤에는 같은 턴에서 진행하지 않고 사용자 다음 응답의 명시 승인을 기다린다.
3. 승인 전에는 Subagent 실행, 파일 수정, 테스트/검증 실행, git/release 작업을 하지 않는다.
4. 실패/재작업 가능성이 큰 영역은 승인 뒤 계획을 잡고 Subagent 배분을 수행한다.
5. 변경 후 결과를 증거로 정리한다.
