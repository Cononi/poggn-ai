...
- 최대 5라운드까지 1회 응답당 질문 1개만 사용해 목표와 범위를 좁힙니다.
...
- 1라운드는 1차 목표나 대표 사용자 시나리오 중 더 불확실한 1개를 확인합니다.
- 2라운드는 포함/제외 범위나 완료 기준 중 구현 방향에 더 큰 영향을 주는 1개를 확인합니다.
- 3라운드는 변경 가능 영역, owner files, forbidden files 중 blocker가 될 1개를 확인합니다.
- 4라운드는 public contract, 데이터, 권한, 보안 결정 중 위험이 가장 큰 1개를 확인합니다.
- 5라운드는 lane 분리나 verification plan 중 남은 blocker 1개만 확정합니다.
- 사용자가 "알아서 해줘"처럼 위임하면 기본값을 선택하되 가정과 위험을 짧게 명시합니다.
- 질문 라운드 후에는 confirmed requirements, non-goals, acceptance criteria, lane contract, verification plan을 요약하고 사용자 확인을 받습니다.
- 5라운드 후에도 핵심 결정이 비어 있으면 구현하지 말고 확정 내용, 미확정 내용, 제안 기본값, 구현 가능 여부를 보고합니다.
...
- blocker가 아닌 scope, architecture, data, auth, API, UX, verification 리스크는 사용자에게 묻지 말고 현재 workflow의 `RISKS.md`에 기록한 뒤 자동 처리합니다.
- API 계약, DB schema, auth 정책, 비용, 외부 서비스, 데이터 삭제, 보안 완화는 confirm 항목으로 분류하고 1문 1답으로 사용자 확인을 받습니다.
- 보안 완화, secret 노출 가능성, 데이터 손실 가능성, owner/tenant 누락은 block 항목으로 분류하고 구현을 멈춥니다.
- 조언은 대화에 메뉴로 나열하지 않고 risk, recommended approach, tradeoff, confirmation need를 현재 workflow의 `RISKS.md`나 현재 workflow의 `DECISIONS.md`에 짧게 남깁니다.
...
- agent 의견은 자유 회의록이 아니라 verdict, risk, recommendation, evidence 형식의 짧은 판정표로 통합합니다.
- main은 충돌 의견을 가장 보수적인 판정으로 정리하고 최종 채택 이유를 현재 workflow의 `DECISIONS.md`에 남깁니다.

## Token Contract

- subagent에는 전체 TASKS.md, 전체 대화, 전체 ready JSON을 전달하지 않습니다.
- 현재 workflow의 `TASKS.md`, `DECISIONS.md`, `RISKS.md`, `RESEARCH.md`, `AGENT_REVIEWS.md` 전체를 context에 넣지 않습니다.
...
- 장부 파일은 사용자 검토용 산출물이며, AI는 summary, trace, files처럼 필요한 조각만 읽습니다.

## Procedure

- 요구사항 계약과 보안 계약이 충족됐는지 확인합니다.
- auto, confirm, block 등급을 정하고 confirm/block 항목만 사용자에게 묻습니다.
...
- agent 판정표, main 결정, 리스크, 최신 기술 검토 결과를 현재 workflow의 장부 파일에 기록합니다.
...
- blocker가 아닌 후속 후보는 사용자에게 메뉴로 묻지 않고 follow-up 후보로 장부에 남깁니다.
- 최신 기술 도입은 필요할 때 research lane으로 확인하고 공식 문서나 릴리즈 근거를 결정 장부에 남깁니다.
...
- DECISIONS.md, RISKS.md, AGENT_REVIEWS.md, 필요한 경우 RESEARCH.md에 짧은 장부 기록이 있습니다.
