...
- `.codex/state/pogo-settings.json`: 커밋되는 기본 설정. CI-safe 기본값을 유지한다.
- `.codex/state/pogo-settings.local.json`: 커밋하지 않는 로컬 override. 실제 실행 상태는 기본 설정 위에 이 파일을 병합한다.
...
`subagent.auto=true`이면 hook은 허용된 git commit/push/merge 전에 `.codex/state/subagent-evidence.json`을 확인한다.
...
2. 상태 확인은 기본 설정 `.codex/state/pogo-settings.json`, 로컬 override `.codex/state/pogo-settings.local.json`, 관련 script 출력을 함께 확인한다.
