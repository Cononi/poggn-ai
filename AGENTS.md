...
- `작업 진행 예정 보고서`는 단순 안내가 아닌 사용자 승인 전 `작업 계약서`다.
- 브리프는 `의도`, `범위`, `제약`, `검증 포인트`를 포함한다.
- 승인 전 STOP 규칙: 보고서 승인 전 Subagent 실행, 파일 수정, 테스트/검증 실행, git/release 작업을 하지 않는다.
- 승인 전 예외는 보고서 작성을 위한 최소 read-only 확인과 상태 조회(`status`/`summary`)만 허용한다.
...
- `subagent-auto`는 훅이 Subagent를 직접 생성하는 기능이 아니라, 메인 오케스트레이터의 우선 위임 정책 플래그다.
- 겹치는 파일은 `integrator` 또는 `worker` 단일 리더가 통합/병합한다.
- 병렬 작업이 어렵거나 충돌 가능성이 높으면 Single Agent로 범위를 한정해 진행한다.
- 병렬 작업이 가능하면 오케스트레이터는 즉시 최소 1개 이상의 Subagent를 기동하고 병렬로 위임한다.
